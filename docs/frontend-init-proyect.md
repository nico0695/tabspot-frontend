...
- [x] **2. Limpiar Boilerplate:** Eliminar el CSS por defecto y limpiar el `app/page.tsx` y `app/layout.tsx`.
- [ ] **3. Absolute Imports:** Verificar en `tsconfig.json` que los paths estén configurados (`"@/*": ["./src/*"]`) para usar rutas limpias. _(diferido a Fase 4 — alias y layout src/ se mueven juntos)_
- [x] **4. Validación de Entorno:** Instalar y configurar `@t3-oss/env-nextjs` con **Zod**. Crear tu archivo `env.ts` para definir qué variables son obligatorias (ej. las de Supabase) para que el proyecto no compile si faltan.
...
- [x] **1. Linter y Formatter:** Instalar **Prettier** y los plugins para integrarlo con ESLint (`eslint-config-prettier`). _(nota: `eslint-plugin-prettier` excluido — perf degradation + doble reporte con flat config)_
- [x] **2. Reglas Estrictas:** Configurar el `.prettierrc` (singleQuote, printWidth:100, trailingComma:all, semi:true). ESLint sigue en `eslint.config.mjs` (flat config).
- [x] **3. Git Hooks:** Instalar e inicializar **Husky** v9. Hook en `.husky/pre-commit`.
- [x] **4. Linting Pre-commit:** Instalar `lint-staged` y configurarlo en el `package.json` (prettier --write + eslint --fix sobre staged files).
